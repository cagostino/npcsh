...
def get_data_response(
    request, db_conn, tables=None, n_try_freq=5, extra_context=None, history=None
):
    prompt = f"""
    ...
    prompt += f"""
            ...
            """
    llm_response = get_llm_response(prompt, format="json")
...
def check_output_sufficient(request, response, query, model=None, provider=None):
    prompt = f"""
    ...
    llm_response = get_llm_response(
        prompt, format="json", model=model, provider=provider
    )
...
def process_data_output(
    llm_response, db_conn, request, tables=None, n_try_freq=5, history=None
):
...
def execute_llm_command(command, command_history, model=None, provider=None):
    max_attempts = 5
    attempt = 0
    subcommands = []
    ...
    while attempt < max_attempts:
        ...
        response = get_llm_response(
            prompt, model=model, provider=provider, format="json"
        )
        ...
        try:
            ...
            fix_suggestion = get_llm_response(
                error_prompt, model=model, provider=provider, format="json"
            )
...
def check_llm_command(command, command_history, model=None, provider=None):
    location = os.getcwd()
    prompt = (
        f"""
    ...
    In considering how to answer this, consider whether it is 
        something that can be answered via a bash command on the users computer? Assume that the user has access to internet 
        and basic command line tools like curl, wget, etc so you are not limited to just the local machine.
    ...
    if response["is_command"] == "yes":
        return execute_llm_command(
            command, command_history, model=model, provider=provider
        )
    else:
...
def execute_llm_question(
    command, command_history, model=None, provider=None, messages=None
):
    location = os.getcwd()
